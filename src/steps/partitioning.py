...
def __mount_boot_partition(partition):
    if partition["format"]:
        response = input("Are you sure that you want to format " +
                         partition["partition"] + " partition?").strip().lower()
        if (response == "y"):
            run_command_with_output(
                ["mkfs.fat", "-F32", partition["partition"]])
    run_command_with_output(
        ["mount", "--mkdir", partition["partition"], "/mnt/boot"])
    ...
    if partition["format"]:
        ...
        if (response == "y"):
            run_command_with_output(
                ["mkfs.fat", "-F32", partition["partition"]])
    run_command_with_output(
        ["mount", "--mkdir", partition["partition"], "/mnt/boot/efi"])
...
def __mount_luks_lvm2_partitions(partition):
    ...
    run_command_with_output(
        "echo -n '" + password + "' | cryptsetup open " + part + " " + crypt_name, shell=True)
